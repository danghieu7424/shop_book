...
pub struct UserResponse {
    ...
    pub student_id: Option<String>, // <-- THÊM MỚI CHO SINH VIÊN
}
...
async fn generate_jwt_response(
    ...
    let exp = chrono::Utc::now()
        .checked_add_signed(chrono::Duration::hours(24)) // Tăng lên 24h
        .unwrap()
        .timestamp() as usize;
    ...
    // Lấy thông tin user bao gồm Student ID
...
// Cập nhật profile (dành cho sinh viên tự cập nhật thông tin)
#[derive(Deserialize)]
...
async fn update_profile(
    ...
    let _ = sqlx::query(
        "UPDATE users SET phone = ?, address = ?, student_id = ? WHERE id = ?"
    )
    .bind(payload.phone)
...
pub fn auth_routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/me", get(get_me).put(update_profile)) // Thêm PUT để update profile
