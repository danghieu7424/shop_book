...
async fn create_order(
    ...
    // SỬA: Lấy giá sale (COALESCE)
    let cart_items: Vec<(String, String, Decimal, i32)> = sqlx::query_as(
        "SELECT p.id, p.name, COALESCE(p.sale_price, p.price) as price, c.quantity 
         FROM cart_items c 
    ...
    let _ = sqlx::query(
        "INSERT INTO orders (id, user_id, total_amount, final_amount, points_earned, status, shipping_name, shipping_phone, shipping_address, note, payment_method) VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?)"
    )
    .bind(&order_id).bind(&auth.user_id).bind(total_amount).bind(total_amount).bind(points_earned).bind(&payload.shipping_info.name).bind(&payload.shipping_info.phone).bind(&payload.shipping_info.address).bind(&payload.shipping_info.note).bind(&payment_method).execute(&mut *tx).await;
...
// ... (Giữ nguyên các hàm my_orders, order_detail, receive_order, cancel_order) ...
...
pub fn order_routes() -> Router<AppState> {
    Router::new().route("/", post(create_order).get(my_orders)).route("/:id", get(order_detail)).route("/:id/receive", put(receive_order)).route("/:id/cancel", put(cancel_order))
}