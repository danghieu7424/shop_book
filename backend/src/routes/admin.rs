...
impl<S> FromRequestParts<S> for AdminUser where S: Send + Sync, AppState: From<S>, S: Clone {
    ...
    async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let role: (String,) = sqlx::query_as("SELECT role FROM users WHERE id = ?").bind(&auth_user.user_id).fetch_one(&app_state.db).await.map_err(|_| (StatusCode::UNAUTHORIZED, Json(serde_json::json!({"message": "User fail"}))))?;
        if role.0 == "admin" { Ok(AdminUser(auth_user.user_id)) } else { Err((StatusCode::FORBIDDEN, Json(serde_json::json!({"message": "Forbidden"})))) }
    }
}

// ... (Giữ nguyên phần Categories) ...
#[derive(Deserialize)]
...
struct CreateProductReq {
    ...
    price: Decimal,
    sale_price: Option<Decimal>, // <-- THÊM: Input giá sale (có thể null)
    stock: i32,
...
async fn create_product(
    ...
    // Insert có cột sale_price
    let res = sqlx::query(
            "INSERT INTO products (id, category_id, name, author, publisher, publication_year, price, sale_price, stock, images, description, specs) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        )
        .bind(id)
...
async fn update_product(
    ...
    // Update có cột sale_price
    let res = sqlx::query(
            "UPDATE products SET category_id=?, name=?, author=?, publisher=?, publication_year=?, price=?, sale_price=?, stock=?, images=?, description=?, specs=? 
             WHERE id=?"
        )
        .bind(payload.category_id)
        .bind(payload.name)
    ...
    match res {
        ...
        Err(_) => (StatusCode::INTERNAL_SERVER_ERROR, Json("Error DB")).into_response()
    }
}
...
// ... (Giữ nguyên phần Orders, Users, Settings, Analytics, Contacts, Config, Router) ...
// (Do file admin.rs rất dài, tôi chỉ hiển thị phần Product đã sửa. Bạn giữ nguyên các phần khác nhé.)

#[derive(Deserialize)]
struct UpdateStatusReq { status: String }
#[derive(Debug, Serialize, FromRow)]
pub struct AdminOrderHistory { pub id: String, pub user_name: Option<String>, pub final_amount: Decimal, pub status: String, pub points_earned: i32, pub created_at: chrono::NaiveDateTime }
...
pub struct SettingItem { pub id: String, pub value: Option<String> }
#[derive(Debug, Deserialize)]
pub struct UpdateSettingReq { pub settings: Vec<SettingItem> }
...
#[derive(Serialize)]
struct AnalyticsData { revenue_month: f64, new_orders: i64, new_users: i64, top_product: String }
async fn get_analytics(State(state): State<AppState>, _: AdminUser) -> impl IntoResponse {
    ...
    let data = AnalyticsData { revenue_month: revenue.0.unwrap_or(Decimal::ZERO).to_f64().unwrap_or(0.0), new_orders: orders_count.0, new_users: users_count.0, top_product: top_prod };
    (StatusCode::OK, Json(data)).into_response()
}
#[derive(Debug, Serialize, FromRow)]
pub struct ContactItem { pub id: String, pub user_id: Option<String>, pub user_name: Option<String>, pub email: String, pub message: String, pub status: String, pub created_at: Option<chrono::NaiveDateTime> }
