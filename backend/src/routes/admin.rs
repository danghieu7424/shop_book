...
async fn create_category(
    ...
    let id = suid();
    let res = sqlx::query("INSERT INTO categories (id, name, description) VALUES (?, ?, ?)")
        .bind(id)
        .bind(payload.name)
    ...
    match res {
        Ok(_) => (StatusCode::CREATED, Json("Tạo danh mục thành công")).into_response(),
        Err(e) => {
            println!("Lỗi tạo danh mục: {:?}", e);
...
async fn delete_category(
    ...
    if count.0 > 0 {
        return (StatusCode::BAD_REQUEST, Json("Không thể xóa: Danh mục này đang chứa sách")).into_response();
    }
...
async fn get_all_orders(State(state): State<AppState>, _: AdminUser) -> impl IntoResponse {
    let orders = sqlx
        ::query_as::<_, AdminOrderHistory>(
            "SELECT o.id, u.name as user_name, o.final_amount, o.status, o.points_earned, o.created_at 
             FROM orders o
...
async fn get_all_users(State(state): State<AppState>, _: AdminUser) -> impl IntoResponse {
    let users = sqlx
        ::query_as::<_, crate::routes::auth::UserResponse>(
            "SELECT id, email, name, picture, role, status, points, level, phone, address, student_id FROM users"
        )
        .fetch_all(&state.db).await;
