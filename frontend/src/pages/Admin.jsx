...
const SimpleBarChart = ({ data }) => {
    const safeData = data || { pending: 0, shipping: 0, completed: 0, cancelled: 0, returned: 0 };
    const total = Object.values(safeData).reduce((a, b) => a + Number(b), 0);
    const getPercent = (val) => total === 0 ? 0 : Math.round((Number(val) / total) * 100);

    const items = [
        { label: "Chờ duyệt", value: safeData.pending, color: "bg-yellow-400", text: "text-yellow-700" },
        { label: "Đang giao", value: safeData.shipping, color: "bg-blue-400", text: "text-blue-700" },
        { label: "Thành công", value: safeData.completed, color: "bg-green-400", text: "text-green-700" },
        { label: "Đã hủy", value: safeData.cancelled, color: "bg-red-400", text: "text-red-700" },
        { label: "Trả hàng", value: safeData.returned, color: "bg-gray-400", text: "text-gray-700" },
    ];
    ...
    return (
        <div className="space-y-4">
            {items.map((item, idx) => (
             ...
             {total === 0 && <div className="text-center text-gray-400 text-sm italic">Chưa có dữ liệu đơn hàng</div>}
        </div>
    );
};
...
const ImageUploader = ({ images = [], onImagesChange, domain }) => {
  ...
  const onDrop = useCallback(
    async (acceptedFiles) => {
      ...
      try {
        const res = await fetch(`${domain}/api/upload`, { method: "POST", body: formData });
        if (res.ok) { onImagesChange([...images, ...await res.json()]); }
  ...
  return (
    <div className="space-y-3">
      <label className="block text-sm font-semibold text-gray-700">Ảnh bìa sách</label>
      <div {...getRootProps()} className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer ${isDragActive ? "border-emerald-500 bg-emerald-50" : "border-gray-300 hover:border-emerald-400"}`}>
        <input {...getInputProps()} />
        <div className="flex flex-col items-center gap-2 text-gray-500">{uploading ? <Loader2 size={32} className="text-emerald-500 animate-spin" /> : <UploadCloud size={32} className="text-emerald-500" />}{uploading ? <span className="animate-pulse text-sm">Đang tải lên...</span> : <span className="text-sm">Kéo thả ảnh vào đây</span>}</div>
      </div>
      {images.length > 0 && (<div className="grid grid-cols-4 gap-3 mt-3">{images.map((link, idx) => (<div key={idx} className="relative group aspect-[3/4] border rounded bg-gray-100"><img src={`${domain}${link}`} className="w-full h-full object-cover" alt="preview" /><button onClick={(e) => { e.stopPropagation(); onImagesChange(images.filter((_, i) => i !== idx)); }} className="absolute top-1 right-1 bg-white text-red-600 p-1 rounded-full shadow-sm hover:bg-red-50" type="button"><X size={14} /></button></div>))}</div>)}
    </div>
  );
};
...
export default function Admin() {
  ...
  // State riêng để theo dõi ID đang được update (tránh disable toàn bộ danh sách)
  const [updatingId, setUpdatingId] = useState(null);

  const fetchData = async (tab = activeTab) => {
    setLoading(true);
    try {
      const [prodRes, catRes] = await Promise.all([fetch(`${domain}/api/products`), fetch(`${domain}/api/categories`)]);
      if (prodRes.ok) setProducts(await prodRes.json());
      if (catRes.ok) dispatch(actions.set_categories(await catRes.json()));
      const opts = { credentials: "include" };
      if (tab === "orders") { const res = await fetch(`${domain}/api/admin/orders`, opts); if (res.ok) setOrders(await res.json()); }
      else if (tab === "users") { const res = await fetch(`${domain}/api/admin/users`, opts); if (res.ok) setUsers(await res.json()); }
      else if (tab === "settings" || tab === "loyalty") { const res = await fetch(`${domain}/api/admin/settings`, opts); if (res.ok) { const data = await res.json(); const setObj = data.reduce((acc, curr) => ({ ...acc, [curr.id]: curr.value }), {}); setSettings(setObj); } }
      else if (tab === "analytics") { const res = await fetch(`${domain}/api/admin/analytics`, opts); if (res.ok) setAnalytics(await res.json()); }
      else if (tab === "contacts") { const res = await fetch(`${domain}/api/admin/contacts`, opts); if (res.ok) setContacts(await res.json()); }
    } catch (e) { console.error(e); }
    setLoading(false);
  };
  ...
  // --- SỬA LỖI: Cập nhật state cục bộ thay vì fetch lại ---
  const updateOrderStatus = async (id, status) => { 
      setUpdatingId(id); // Chỉ hiện loading cho dòng này
      try { 
          const res = await fetch(`${domain}/api/admin/orders/${id}/status`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }), credentials: "include" }); 
          if(res.ok) {
              // Cập nhật trực tiếp vào state orders để không phải load lại
  ...
  // --- SỬA LỖI: Cập nhật contact cục bộ ---
  const updateContactStatus = async (id, status) => { 
      setUpdatingId(id);
      try { 
          const res = await fetch(`${domain}/api/admin/contacts/${id}/status`, { method: 'PUT', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ status }), credentials: 'include' }); 
          if(res.ok) {
  ...
  const renderContent = () => {
    if (loading) return <div className="flex flex-col h-64 items-center justify-center text-emerald-600"><Loader2 size={40} className="animate-spin mb-2" /><span>Đang tải dữ liệu...</span></div>;
    ...
    switch (activeTab) {
      ...
      case "contacts": return (<div className="space-y-4 animate-fade-in"><h2 className="text-2xl font-bold">Hòm thư góp ý</h2>{contacts.map(c => (<Card key={c.id} className="p-4 flex gap-4"><div className="flex-1"><div className="font-bold text-gray-800 flex items-center gap-2"><Mail size={14}/> {c.email} <span className="text-xs font-normal text-gray-400">({new Date(c.created_at).toLocaleDateString()})</span></div><p className="bg-gray-50 p-3 mt-2 rounded italic text-gray-700">"{c.message}"</p></div><div className="flex flex-col gap-2 min-w-[120px]"><Badge color={c.status === "processed" ? "green" : "yellow"}>{c.status === "processed" ? "Đã trả lời" : "Chờ xử lý"}</Badge>{c.status === "new" && (<Button size="sm" onClick={() => updateContactStatus(c.id, "processed")} className="bg-emerald-600 text-white text-xs" disabled={updatingId === c.id}>{updatingId === c.id ? "..." : "Đánh dấu xong"}</Button>)}<a href={`mailto:${c.email}`} className="text-xs text-blue-600 text-center hover:underline">Gửi mail trả lời</a></div></Card>))}{contacts.length === 0 && <div className="text-center text-gray-500 py-10 border border-dashed rounded bg-gray-50">Hòm thư trống</div>}</div>);
      default: return null;
    }
  };

  const menuItems = [
    { id: "categories", label: "Quản lý Danh mục", icon: <Layers size={20} /> }, { id: "products", label: "Kho sách", icon: <BookOpen size={20} /> }, { id: "orders", label: "Đơn đặt hàng", icon: <ShoppingCart size={20} /> }, { id: "users", label: "Sinh viên", icon: <Users size={20} /> }, { id: "loyalty", label: "Điểm thưởng", icon: <Star size={20} /> }, { id: "analytics", label: "Thống kê", icon: <BarChart2 size={20} /> }, { id: "settings", label: "Cấu hình", icon: <Settings size={20} /> }, { id: "contacts", label: "Góp ý", icon: <MessageSquare size={20} /> }
  ];

  return (
    <div className="flex min-h-screen bg-gray-50 font-sans">
      <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r shadow-xl transform transition-transform duration-300 md:translate-x-0 md:static ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="p-6 border-b flex items-center gap-2 font-bold text-xl text-gray-800"><LayoutDashboard className="text-emerald-600" /> AdminPanel</div>
        <nav className="p-4 space-y-2">{menuItems.map((item) => (<button key={item.id} onClick={() => { if (submitting) return; setActiveTab(item.id); setSidebarOpen(false); setIsEditingProd(false); }} disabled={submitting} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all font-medium ${activeTab === item.id ? "bg-emerald-600 text-white shadow-md" : "text-gray-600 hover:bg-gray-100"} ${submitting ? "opacity-50 cursor-not-allowed" : ""}`}>{item.icon}{item.label}</button>))}</nav>
      </div>
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <div className="md:hidden p-4 bg-white border-b flex justify-between items-center"><span className="font-bold">Menu</span><button onClick={() => setSidebarOpen(!sidebarOpen)}><Menu /></button></div>
