...
import { X, QrCode, CheckCircle, Crown, ShoppingCart, User, Save, Phone, MapPin, LogOut, Wallet, Ban } from "lucide-react";
...
export default function Profile() {
  ...
  const handleCancelOrder = async (orderId) => {
      ...
      try {
          const res = await fetch(`${domain}/api/orders/${orderId}/cancel`, { method: "PUT", credentials: "include" });
          if(res.ok) { alert("Đã hủy đơn hàng thành công!"); fetchOrders(); } 
          else { const err = await res.json(); alert("Lỗi: " + (err || "Không thể hủy đơn")); }
  ...
  const handleReceived = async (orderId) => {
    ...
    try {
      ...
      if (res.ok) { alert("Xác nhận thành công! Điểm thưởng đã được cộng."); fetchOrders(); } 
      else { const err = await res.json(); alert("Lỗi: " + (err || "Không thể xác nhận")); }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="grid md:grid-cols-3 gap-8">
        <div className="space-y-6">
          <Card className="p-0 overflow-hidden border-none shadow-lg">
             ...
             <div className="px-6 pb-6 relative">
                 <div className="flex justify-center mt-[-48px]"><img src={userInfo.picture || "https://via.placeholder.com/100"} className="w-24 h-24 rounded-full border-4 border-white shadow-md object-cover bg-white" alt="avatar" /></div>
                  <div className="mt-16 text-center"><h2 className="font-bold text-xl text-gray-800">{userInfo.name}</h2><div className="text-sm text-gray-500 mb-2">{userInfo.email}</div><Badge color="blue">{LEVELS[userInfo.level]?.name || userInfo.level} Reader</Badge></div>
             </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border p-6"><div className="flex justify-between items-end mb-4"><div><div className="text-gray-500 text-sm font-medium flex items-center gap-1"><Crown size={16} className="text-yellow-500" /> Điểm tích lũy</div><div className="text-4xl font-extrabold text-emerald-700 mt-1">{userInfo.points} <span className="text-sm font-normal text-gray-400">pts</span></div></div></div>{loyaltyStatus && loyaltyStatus.nextLevel && (<div><div className="w-full bg-gray-100 rounded-full h-2.5 mb-2 overflow-hidden"><div className="bg-gradient-to-r from-emerald-500 to-teal-500 h-2.5 rounded-full" style={{ width: `${loyaltyStatus.progress}%` }}></div></div><div className="text-xs text-gray-500 text-center">Cần thêm <b>{loyaltyStatus.pointsNeeded}</b> pts để lên hạng <b>{loyaltyStatus.nextLevel.name}</b></div></div>)}</div>
        </div>
            ...
            {Array.isArray(orders) && orders.map((o) => (
              <Card key={o.id} className="p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-md transition-shadow">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1"><div className="font-bold text-lg text-gray-800">Đơn #{o.id.substring(0, 8)}</div><Badge color={o.status === "completed" ? "green" : o.status === "shipping" ? "blue" : o.status === "cancelled" ? "red" : "yellow"}>{o.status === "completed" ? "Hoàn thành" : o.status === "shipping" ? "Đang giao" : o.status === "pending" ? "Chờ duyệt" : "Đã hủy"}</Badge></div>
